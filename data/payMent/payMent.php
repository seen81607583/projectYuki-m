...
$data = [];
 if($uid){
    $sql = "SELECT uname,email,phone,uid,address FROM yuki_user WHERE uid=$uid";
    $result = mysqli_query($conn,$sql);
    $data["user"] = mysqli_fetch_all($result,MYSQLI_ASSOC)[0];
     ...
     WHERE uid=$uid AND isChecked=0 AND cid IN ($cids)";
     $result = mysqli_query($conn,$sql);
     $data["products"] = mysqli_fetch_all($result,MYSQLI_ASSOC);
